...
4. **`pyproject.toml` (Required)**: Package dependencies and python configurations.
   - **Static Configuration Naming Sync:** You MUST manually sync the name/id placeholder fields in:
     * `pyproject.toml` (project `name` metadata)
     * `agents-cli-manifest.yaml` (`name` field)
     * `deployment/terraform/single-project/vars/env.tfvars` (`project_name` variable)
     * `deployment/terraform/single-project/variables.tf` (defaults for `project_name`, `telemetry_logs_filter`, and `feedback_logs_filter`)
